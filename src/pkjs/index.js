    ...
    function (responseText) {
      ...
      for (var k in json) {
        ...
        if (date.getHours() == dict[keys.FavouriteHour] - timezoneOffsetHours) {
          ...
          dictionary[keys.SwellPeriod + num] = json[k].swell.components.primary.period
          dictionary[keys.SwellHeight] +=
            (Math.round((json[k].swell.maxBreakingHeight + json[k].swell.minBreakingHeight) / 2 * 10) / 10)
              .toString()
          dictionary[keys.SwellDirection + num] = Math.round(json[k].swell.components.primary.direction)
          // Wind Keys
