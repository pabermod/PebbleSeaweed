...
static void inbox_received_callback(DictionaryIterator *iterator, void *context) {
  ...
  if(fav_hour_tuple){
    ...
    APP_LOG(APP_LOG_LEVEL_INFO, "Favourite Hour is %s", favhour_buffer);
    settings.FavouriteHour = atoi(favhour_buffer);
    save_settings();
  }
  ...
  if(period_tuple && heights_tuple) {
    ...
    snprintf(height_buffer, sizeof(height_buffer), "%dm", (int)heights_tuple->value->int32);
    ...
    snprintf(forecast_layer_buffer, sizeof(forecast_layer_buffer), "%s, %s", height_buffer, period_buffer);
    text_layer_set_text(s_weather_layer, forecast_layer_buffer);
  }
}
